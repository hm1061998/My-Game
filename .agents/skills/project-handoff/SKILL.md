...
Read `AGENTS.md`, `docs/agent/protocol.md`, `docs/agent/improvement.md`, `docs/memory/current.md`, the active task, and relevant nested rules. Inspect open lessons relevant to the task. Compare their claims with Git status and the actual files before acting.
...
- the improvement review result and any lesson/rule/skill validation;
- one concrete next action and only the references needed for it.

Do not copy chat transcripts, secrets, raw tokens, large logs, or speculative conclusions into memory. A task is `done` only when its acceptance, browser evidence or justified N/A, required scripts, and improvement review are recorded.
