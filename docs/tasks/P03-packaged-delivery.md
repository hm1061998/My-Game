...
Workflow step: scanner-route correction passed remotely; session-start diagnostic added after a separate intermittent UI timeout; waiting on remote CI for the instrumented test
...
- Added direct assertions for the session-start POST response (HTTP 201 and revision 0) before the UI assertion so a repeat distinguishes API failure from UI state propagation. Local headed Chromium passed 4/4 and full `verify.ps1` passed again. Remote CI for this diagnostic revision is pending.
- Diagnostic CI run `36236947169` on `3bd397873ebf6b56517ebd463a3fdf649c69646a` confirmed session creation completed and then exposed two slow-renderer E2E assumptions: the retained up-vector made dodge hit the cabinet's right edge (`x=1119.68`) before the test tried to recenter; the missing-texture visual test used a fixed 300 ms key hold and sometimes observed no movement. Updated the journey to set/rightward-assert movement before dodging and continue through the corridor, and changed the visual check to poll actual position with key-up in `finally`. Local visible headed E2E passes 4/4; full `verify.ps1` passes. Remote rerun is pending.
...
- CI run `36233739417` for P03 package/evidence SHA `a63e3072eb26efe83939ff4353f06c28441165dd`: `verify`, `e2e`, and `docker` green. Run `36234356486` exposed an E2E corridor-targeting defect; route-correction SHA `98448555c85815d0523c5ced457080b5c0b2da6d` passed all three jobs in run `36235584949`. Run `36236199020` had a startup UI timeout; instrumented run `36236947169` proved startup passed, then exposed dodge-vector and fixed-wait test assumptions. Require green CI on the final movement-aware test revision before closing P03.
...
The package, security and persistence acceptance is complete. Remote reruns exposed slow-runner E2E direction/fixed-wait assumptions; the journey now observes create-session response, sets dodge direction deliberately, and polls movement. Local headed E2E and full scripts pass, but keep P03/`delivery_complete` open until remote CI passes on this revision. Stop before deploy/publish or GitHub settings changes.

## Handoff

Last pushed commit `3bd397873ebf6b56517ebd463a3fdf649c69646a` on `origin/main`; dirty paths for this follow-up: `apps/web/AGENTS.md`, `apps/web/e2e/critical-journey.spec.ts`, `apps/web/e2e/visual-states.spec.ts`, `docs/agent/lessons.md`, `docs/memory/current.md`, `docs/quality/P03-delivery-readiness.md`, `docs/tasks/P03-packaged-delivery.md`, and `docs/tasks/index.md`. These are test/agent guidance/evidence only; gameplay/runtime behavior is unchanged. Plan 1.0 approval remains the user's 2026-09-26 “duyệt plan”.
...
Failed/not-run notes: initial bare `bash` invocation did not run the smoke because its WSL shim had no `/bin/bash`; explicit Git Bash passed. The first headed E2E launch was not started because automatic permission review timed out; a single retry ran successfully. Codex in-app browser automation also hit the Windows sandbox helper error; the package journey was completed in visible Chrome instead. Closeout run `36234356486` failed E2E at the collider boundary; route fix passed all jobs in `36235584949`. Run `36236199020` timed out at session-start UI, while instrumented run `36236947169` passed session start then identified dodge-direction and fixed-hold assumptions. These were fixed and pass local headed 4/4/full verify; remote CI is pending.

Next action: commit and push the session-start diagnostic plus this task checkpoint, observe the associated CI, and close P03 only after it passes. Preserve default stopped containers and `office-case-files_ocf-data`; no deployment, image publication or repository settings changes.
...
- Validation: headed E2E 4/4 and full verify passed locally; route-correction revision passed all three jobs in run `36235584949`. Later runs confirmed session start but exposed retained dodge direction and a timed movement assertion; fixed locally, remote rerun required.
- Follow-up trigger: revalidate if scanner/cabinet geometry, dodge direction semantics, or E2E movement tolerances change.
