...
Sao lưu/khôi phục tiến độ (`--export` / `--import`), reset và kiểm tra bàn giao: xem [docs/runbooks/local.md](docs/runbooks/local.md). Chạy bằng Docker (`docker compose up -d`, mở `http://127.0.0.1:8080`): xem mục 7 của runbook. CI trên GitHub và khuyến nghị bảo vệ nhánh: xem [docs/runbooks/github.md](docs/runbooks/github.md).
