...
test('complete case survives retry, reload and replay', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('dialog', { name: 'Kết quả máy quét' })).toContainText('Bị máy quét phát hiện')
  ...
  expect((await snapshot(page))?.dodgeRemainingMs).toBeGreaterThan(0)
