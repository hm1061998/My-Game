...
export class GameAudio {
  ...
  private createContext() {
    ...
    this.createAmbienceBuffer()
    this.applyLevels()
  }

  private createAmbienceBuffer() {
    const context = this.context
  ...
  private applyLevels() {
    ...
    this.ambienceGain.gain.setTargetAtTime(this.preferences.ambience ? 0.035 : 0, now, 0.08)
  ...
  private syncAmbience() {
    ...
    if (this.ambienceSource && this.ambienceStarted) return
    if (!this.ambienceSource) this.createAmbienceBuffer()
    const source = this.ambienceSource
    source?.start()
    this.ambienceStarted = !!source
  }
